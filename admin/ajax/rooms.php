    ...
    if(isset($_POST['add_room'])){
        ...
        if($stmt2 = mysqli_prepare($conn, $q2)){
            foreach ($facilities as $fa) {
                mysqli_stmt_bind_param($stmt2, 'ii', $room_id, $fa);
            ...
            $flag = 0;
            die('querry can not run - insert');
        }
        
        
       
        $q3 = "INSERT INTO `room_features`(`room_id`, `features_id`) VALUES (?,?)";
        if($stmt3 = mysqli_prepare($conn, $q3)){
            foreach($features as $fe){
                mysqli_stmt_bind_param($stmt3,'ii', $room_id, $fe); 
...
//check flag tu day
        ...
        if($stmt2 = mysqli_prepare($conn, $q2)){
            foreach ($facilities as $fa) {
                mysqli_stmt_bind_param($stmt2, 'ii', $frm_data['room_id'], $fa);
                mysqli_stmt_execute($stmt2);
            }
            $flag = 1;
            mysqli_stmt_close($stmt2);
        ...
        if($stmt3 = mysqli_prepare($conn, $q3)){
            foreach($features as $fe){
                mysqli_stmt_bind_param($stmt3,'ii', $frm_data['room_id'], $fe); 
                mysqli_stmt_execute($stmt3);
            }
            $flag = 1;
            mysqli_stmt_close($stmt3);
